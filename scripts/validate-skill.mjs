...
function validateSkill(skillDir) {
	...
	assert(
		keys.every((key) => ["name", "description"].includes(key)),
		`Frontmatter should only contain name and description for maximum Codex/Pi/Claude/Grok compatibility; found: ${keys.join(", ")}`,
		errors,
	);
	...
	assert(
		...
		"Description should be specific enough to trigger correctly",
		errors,
	);
	assert(
	...
	assert(
		/Use (for|when)/.test(description),
		"Description should include explicit use/trigger guidance",
		errors,
	);
	assert(
		!/\[TODO\]|TODO:/i.test(markdown),
		"Skill must not contain TODO placeholders",
		errors,
	);
	...
	const requiredBodyNeedles = [
		...
		"command -v gemini",
		"command -v agy",
		"fallback to the current harness itself",
		"gemini_api_key",
		"ai_gateway_api_key",
		"vercel-ai-gateway",
		"pi --list-models",
	...
	for (const needle of requiredBodyNeedles) {
		assert(
			...
			`Body missing required workflow concept: ${needle}`,
			errors,
		);
	}
	...
	for (const agent of agentNames) {
		assert(
			...
			`Body should mention ${agent} compatibility`,
			errors,
		);
	}
	...
	if (existsSync(openaiYaml)) {
		...
		assert(
			yaml.includes("display_name:"),
			"agents/openai.yaml missing interface.display_name",
			errors,
		);
		assert(
			yaml.includes("short_description:"),
			"agents/openai.yaml missing interface.short_description",
			errors,
		);
		assert(
			yaml.includes(`$${name}`),
			`agents/openai.yaml default_prompt should mention $${name}`,
			errors,
		);
	}
...
try {
	...
	if (errors.length) {
		console.error(
			`Skill validation failed (${errors.length} issue${errors.length === 1 ? "" : "s"}):`,
		);
