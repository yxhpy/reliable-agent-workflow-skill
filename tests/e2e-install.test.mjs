...
function installerTestEnv(extraEnv = {}) {
...
function assertInstalled(home, agent) {
	...
	assert.ok(
		...
		`${agent} SKILL.md should be installed`,
	);
	...
	try {
		...
		const bin = path.join(
			...
			process.platform === "win32"
				? "install-reliable-agent-workflow.cmd"
				: "install-reliable-agent-workflow",
		);
		if (process.platform === "win32") {
			execFileSync(
				"cmd.exe",
				["/c", bin, "--agent", "codex", "--home", home, "--force"],
				{
					encoding: "utf8",
			...
			execFileSync(bin, ["--agent", "codex", "--home", home, "--force"], {
				encoding: "utf8",
				env: installerTestEnv(),
			});
		}
