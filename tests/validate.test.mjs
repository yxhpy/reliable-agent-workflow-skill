...
test("skill validates against local compatibility rules", () => {
	const output = execFileSync(
		process.execPath,
		["scripts/validate-skill.mjs", "reliable-agent-workflow"],
		{
			cwd: repoRoot,
			encoding: "utf8",
		},
...
test("skill encodes frontend Gemini/Agy CLI routing with safe fallback", () => {
	const markdown = readFileSync(
		path.join(skillDir, "SKILL.md"),
		"utf8",
	).toLowerCase();
	...
	for (const needle of required) {
		assert.ok(
			...
			`missing Pi model-routing pattern: ${needle}`,
		);
	}
});
